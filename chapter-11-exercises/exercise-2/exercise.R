...
nrow(distinct(vehicle_makes))

# Filter the data set for vehicles manufactured in 1997
vehicles_1997 <- filter(vehicles, year == 1997)

# Arrange the 1997 cars by highway (`hwy`) gas milage
by_gas_mileage <- arrange(vehicles_1997, hwy)
...
two_wheel_20_cty <- filter(vehicles, drive == "2-Wheel Drive", cty > 20)

# Of the above vehicles, what is the vehicle ID of the vehicle with the worst 
# hwy mpg?
# Hint: filter for the worst vehicle, then select its ID.
worst_hwy <- filter(two_wheel_20_cty, hwy == min(hwy))
select(worst_hwy, id)
...
best_mpg_hwy <- function(year_choice, make_choice) {
  vehicles_year_make <- vehicles %>%
    filter(year == year_choice) %>%
    filter(make == make_choice)
  
  best_mpg <- filter(vehicles_year_make, hwy == max(hwy)) %>%
    select(model)
}
